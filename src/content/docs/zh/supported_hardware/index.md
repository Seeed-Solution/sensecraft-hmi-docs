---
title: 支持的硬件
description: SenseCraft HMI 兼容设备完整列表，包括规格、产品预览和计划支持情况。
---

SenseCraft HMI 支持越来越多的 ePaper 显示设备。以下是按产品系列整理的完整兼容性概览。

## 兼容性表格

| 设备 | 显示尺寸 | 颜色 | 分辨率 |
|---|---|---|---|
| reTerminal E1001 | 7.5" | 单色（4 级灰度） | 800×480 |
| reTerminal E1002 | 7.3" | 全彩（Spectra 6） | 800×480 |
| reTerminal E1003 | 10.3" | 单色（16 级灰度） | 1404×1872 |
| reTerminal E1004 | 13.3" | 全彩（Spectra 6） | 1200×1600 |
| XIAO EE02 ePaper DIY Kit | 13.3" | 全彩（Spectra 6） | 1200×1600 |
| XIAO EE03 ePaper DIY Kit | 10.3" | 单色（16 级灰度） | 1404×1872 |
| XIAO ePaper DIY Kit EE04 | 1.54"-7.5" | 多色 | 200×200 至 800×480 |
| TRMNL 7.5" (OG) DIY Kit | 7.5" | 单色（4 级灰度） | 800×480 |

---


### reTerminal E1001

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/24.jpg" style={{width:700, height:'auto'}}/></div><br />

<div class="get_one_now_container" style={{textAlign: 'center'}}>
  <a
    class="get_one_now_item"
    href="https://wiki.seeedstudio.com/getting_started_with_reterminal_e1001"
    target="_blank"
    rel="noopener noreferrer"
  >
    <strong><span><font color={'FFFFFF'} size={"3"}>reTerminal E1001 Wiki</font></span></strong>
  </a>
  <a
    class="get_one_now_item"
    href="https://www.seeedstudio.com/reTerminal-E1001-p-6534.html"
    target="_blank"
    rel="noopener noreferrer"
  >
    <strong><span><font color={'FFFFFF'} size={"4"}>立即购买</font></span></strong>
  </a>
</div><br />

#### 简介
reTerminal E1001 配备 **7.5 英寸 800×480 ePaper 显示屏**，支持 **4 级灰度**。它能够提供清晰、易读的显示效果，同时保持极低功耗，非常适合对续航有较高要求的应用场景。

#### 核心特性
- 7.5 英寸单色 ePaper 显示屏，支持 4 级灰度
- 800×480 分辨率
- 超低功耗，电池续航更长
- 非常适合日历、待办事项和信息展示等以文本为主的应用


### reTerminal E1002

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/25.jpg" style={{width:700, height:'auto'}}/></div><br />

<div class="get_one_now_container" style={{textAlign: 'center'}}>
  <a class="get_one_now_item" href="https://wiki.seeedstudio.com/getting_started_with_reterminal_e1002" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>reTerminal E1002 Wiki</font></span></strong>
  </a>
  <a class="get_one_now_item" href="https://www.seeedstudio.com/reTerminal-E1002-p-6533.html" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>立即购买</font></span></strong>
  </a>
</div><br />

#### 简介
reTerminal E1002 配备 **7.3 英寸 ACeP 全彩 ePaper 显示屏**。它在保持超低功耗的同时，还能提供鲜艳的多色显示效果，非常适合需要清晰视觉区分的美观型应用。

#### 核心特性
- 7.3 英寸全彩 ePaper 显示屏，采用 ACeP 技术
- 800×480 分辨率
- 在保持低功耗的同时提供丰富色彩表现
- 适用于画廊展示、彩色仪表板和更注重视觉效果的应用

### reTerminal E1003

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/e1003/2-reTerminal-E1003-Epaper-Display.jpg" style={{width:700, height:'auto'}}/></div><br />

<div class="get_one_now_container" style={{textAlign: 'center'}}>
  <a class="get_one_now_item" href="https://wiki.seeedstudio.com/getting_started_with_reterminal_e1003" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>reTerminal E1003 Wiki</font></span></strong>
  </a>
  <a class="get_one_now_item" href="https://www.seeedstudio.com/catalogsearch/result/?q=e1003" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>立即购买</font></span></strong>
  </a>
</div><br />

#### 简介
reTerminal E1003 是一款 **10.3 英寸（1404×1872）16 级灰度 ePaper 显示设备**，基于 **ESP32-S3**。它原生支持 **SenseCraft HMI（无代码）**，并在 v1.1.2 固件中启用了触摸交互，同时仍兼容 Arduino 和 PlatformIO。未来更新还将支持开源触摸库和 ESPHome 驱动。

#### 核心特性
- 10.3 英寸单色 ePaper 显示屏，在 HMI 固件 v1.1.2 中支持触摸交互
- 16 级灰度，1404×1872 高分辨率
- 超低功耗，续航最高可达 6 个月

### reTerminal E1004

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/e1004/2-reterminal-e1004-epaper-display.jpg" style={{width:700, height:'auto'}}/></div><br />

<div class="get_one_now_container" style={{textAlign: 'center'}}>
  <a class="get_one_now_item" href="https://wiki.seeedstudio.com/getting_started_with_reterminal_e1004" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>reTerminal E1004 Wiki</font></span></strong>
  </a>
  <a class="get_one_now_item" href="https://www.seeedstudio.com/reTerminal-E1004-p-6692.html" target="_blank" rel="noopener noreferrer">
    <strong><span><font color={'FFFFFF'} size={"4"}>立即购买</font></span></strong>
  </a>
</div><br />

#### 简介
reTerminal E1004 配备 **13.3 英寸全彩 ePaper 显示屏**，基于 **ESP32-S3**。它支持 **SenseCraft HMI** 进行无代码仪表板设计和图像上传，同时完全兼容 **Home Assistant**、Arduino 和 ESP-IDF 开发。

#### 核心特性
- 13.3 英寸全彩 ePaper 显示屏
- 适合常亮场景，续航最高可达 6 个月
- 兼容 Home Assistant、Arduino 和 ESP-IDF

---

### XIAO ePaper Display Board EE02

<div class="table-center">
<table align="center">
    <tr>
        <th>XIAO ePaper Display Board EE02</th>
    </tr>
    <tr>
    <td><div align="center"><img width ={300} src="https://files.seeedstudio.com/wiki/hmi/img/ee02.png"/></div>
    <div class="get_one_now_container" style={{textAlign: 'center'}}>
        <a class="get_one_now_item" href="https://www.seeedstudio.com/XIAO-ePaper-DIY-Kit-EE02-for-13-3-Spectratm-6-E-Ink.html" target="_blank">
                <strong><span><font color={'FFFFFF'} size={"4"}> 立即购买</font></span></strong>
        </a>
    </div></td>
    </tr>
 </table>
</div>

#### 简介
**EE02** 是一款高性能 **ESP32-S3** 驱动板，专为 **13.3 英寸** E-Ink 显示屏优化。它集成了 Wi-Fi 和蓝牙，可在 IoT 应用中实现便捷的无线连接。EE02 简化了大尺寸屏幕的驱动流程，并完整支持 **SenseCraft HMI** 平台，方便进行无代码内容管理。

#### 特性

- **高性能控制器**：基于 ESP32-S3，具备强大的处理能力和无线连接能力
- **面向大尺寸屏幕优化**：专为驱动 13.3 英寸 E-Ink 显示屏设计，稳定性高、显示清晰
- **无线连接**：支持 Wi-Fi 和 Bluetooth 5.0（LE），便于远程数据更新和云端集成
- **低功耗设计**：适合电池供电场景
- **丰富接口**：包含 Type-C 供电/编程接口，以及适用于 E-Ink 屏幕的标准连接接口

---

### XIAO ePaper Display Board EE03

<div class="table-center">
<table align="center">
    <tr>
        <th>XIAO ePaper Display Board EE03</th>
    </tr>
    <tr>
    <td><div align="center"><img width ={300} src="https://files.seeedstudio.com/wiki/hmi/img/ee03.png"/></div>
    <div class="get_one_now_container" style={{textAlign: 'center'}}>
        <a class="get_one_now_item" href="https://www.seeedstudio.com/XIAO-ePaper-DIY-Kit-EE03-for-10-3-Monochrome-ePaper-Display.html" target="_blank">
                <strong><span><font color={'FFFFFF'} size={"4"}> 立即购买</font></span></strong>
        </a>
    </div></td>
    </tr>
 </table>
</div>

#### 简介

这款 ePaper 驱动板 EE03 采用 **XIAO ESP32-S3 Plus**，支持 **10.3 英寸高刷新 ePaper 显示屏**。它配备 JST 2.0 mm 电池接口和电源开关、板载充电 IC，并带有 1 个复位按键和 3 个用户按键。非常适合数码相框和信息看板等低功耗 ePaper 项目。

#### 特性

- **内置 XIAO ESP32-S3 Plus：** 连接兼容的 ePaper 显示屏后即可使用。
- **专为 10.3 英寸单色 ePaper 显示屏设计：** 适用于 10.3 英寸高速刷新 ePaper 显示屏，部署快速且运行稳定。
- **支持高速刷新：** 集成 T-CON 时序控制，可实现精确高速驱动；板载 SHT40 可实时感知温度并进行波形补偿。
- **带开关的 BAT 接口：** 方便连接电池，并支持电源管理和节能。
- **便捷用户按键：** 配备 1 个复位按键和 3 个用户可编程按键，便于项目开发和功能定制。

---

### ePaper DIY Kit EE04

<div class="table-center">
<table align="center">
    <tr>
        <th>XIAO ePaper Display Board EE04</th>
    </tr>
    <tr>
    <td><div align="center"><img width ={300} src="https://files.seeedstudio.com/wiki/hmi/img/ee04.png"/></div>
    <div class="get_one_now_container" style={{textAlign: 'center'}}>
        <a class="get_one_now_item" href="https://www.seeedstudio.com/XIAO-ePaper-EE04-DIY-Bundle-Kit.html" target="_blank">
                <strong><span><font color={'FFFFFF'} size={"4"}> 立即购买</font></span></strong>
        </a>
    </div></td>
    </tr>
 </table>
</div>

#### 简介

EE04 显示驱动板采用 **XIAO ESP32-S3 Plus**，支持 **24-pin** 和 **50-pin** 两类 ePaper 显示屏。它配备 JST 2.0 mm 电池接口和电源开关、板载充电 IC，并带有 1 个复位按键和 3 个用户按键。非常适合数字标牌、电子标签和便携式信息看板等低功耗 ePaper 项目。

#### 特性

- **采用 XIAO ESP32-S3 Plus：** 连接兼容的 ePaper 显示屏后即可使用。
- **广泛的显示兼容性：** 兼容多种 ePaper 屏幕，支持 24-pin 和 50-pin 接口，并可通过跳线帽轻松切换。
- **带开关的 BAT 接口：** 方便连接电池，并支持高效电源管理和节能。
- **便捷用户按键：** 配备 1 个复位按键和 3 个用户可编程按键，便于项目开发和功能扩展。

---

### TRMNL 7.5" (OG) DIY Kit

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/XIAO_Gadget/TRMNL_Kit_Pic/head.jpg" style={{width:1000, height:'auto'}}/></div>

<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://www.seeedstudio.com/TRMNL-7-5-Inch-OG-DIY-Kit-p-6481.html" target="_blank">
            <strong><span><font color={'FFFFFF'} size={"4"}> 立即购买</font></span></strong>
    </a>
</div>

#### 简介

**TRMNL 7.5" (OG) DIY Kit** 是一款开放硬件 ePaper 套件，配备 7.5 英寸单色显示屏。您可以将 SenseCraft HMI 刷入该套件，作为默认 TRMNL 固件的无代码替代方案。

#### 特性

- **TRMNL BYOD 兼容性**：官方套件，可用于构建 TRMNL 7.5" OG 设备，支持 375+ 插件和 8 种无代码布局（例如 Calendar、Shopify、GitHub）
- **7.5" E-Ink 显示屏**：800×480 单色显示，支持局部刷新（0.34s）和全屏刷新（3.5s）
- **节能设计**：2000mAh 电池在深度休眠模式下（6 小时刷新一次）续航最长可达 3 个月
- **双生态支持**：兼容 Home Assistant（通过 ESPHome）和 Arduino IDE，便于自定义开发
- **开源外壳资源**：可在 Printables 和 Thingiverse 上获取，方便个性化定制

---
